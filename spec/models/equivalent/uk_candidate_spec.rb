...
RSpec.describe Equivalent::UkCandidate do
  ...
  let(:no_address_line_1) { build(:equivalent_uk_candidate, address_line_1: "") }
  ...
  describe "validation" do
    ...
    context "with invalid postcodes" do
      ['giib', 'tr111 1uf', 'FFFF', 'blahblah', 'p3000 xxx'].each do |invalid_postcode|
        let(:instance) { build(:equivalent_uk_candidate, postcode: invalid_postcode) }
        it "is not valid" do
    ...
    context "with valid postcodes" do
      ['eh3 9eh', 'tr1 1xy', 'hs1 3eq'].each do |valid_postcode|
        let(:instance) { build(:equivalent_uk_candidate, postcode: valid_postcode) }
        it "is valid" do
