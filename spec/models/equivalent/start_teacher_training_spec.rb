  ...
  describe "validation" do
    context "with invalid subject options" do
      ['skiing', '2030', '1995', 'surfing' ].each do |invalid_year|
        let(:instance) { build(:equivalent_start_teacher_training, year_of_entry: invalid_year) }
        it "is not valid" do
    ...
    context "with valid subject options" do
      ['2020', '2021', '2022'].each do |valid_year|
        let(:instance) { build(:equivalent_start_teacher_training, year_of_entry: valid_year) }
        it "is valid" do
