...
def add_stub(uri, response_body)
  stub_request(:get, uri)
    .to_return(body: JSON.generate(response_body), headers: {
...
RSpec.feature "Dependency contracts", :vcr, type: :feature do
  ...
  before do
    ...
    add_stub latest_privacy_policy_api, privacy_policy_body
    add_stub specific_privacy_policy_api, privacy_policy_body
    add_stub degree_status_api, degree_status_body
    add_stub teaching_subjects_api, teaching_subjects_body
    add_stub countries_api, countries_body
    add_stub uk_degree_grades_api, uk_degree_grades_body
    add_stub education_phases_api, education_phases_body
    add_stub retake_gcse_status_api, retake_gcse_status_body
    add_stub initial_teacher_training_years_api, initial_teacher_training_years_body
    add_stub qualification_types_api, qualification_types_body
    add_stub booking_quotas_api, booking_quotas_body
  ...
  contract_fixture_files.each do |filename|
    ...
    fixture_data = data["uiSignUp"]

    next if fixture_data["Persona"] == "--" || fixture_data["Final Journey Status"] != "COMPLETE"

    # add the candidate to the correct Persona list
    personas[fixture_data["Persona"]] = personas[fixture_data["Persona"]] || []
    personas[fixture_data["Persona"]].push data
  end

  personas.each do |persona, candidates|
    feature "As a #{persona}" do
      candidates.each do |fixture|
        candidate = fixture["uiSignUp"]
        api_submission = fixture["apiSubmission"]

        scenario "Someone can sign up for a Teacher Training Advisor with profile defined by [#{fixture['filename']}]" do
          ...
          expect(request).to eql api_submission
        end
      end
    end
  end
